...
- 開発（Claude）では完結できず、**ユーザー（オーナー／運営者）側の判断・情報・外部手続きが必要な作業**が発生したら、**必ず `docs/オーナー作業TODO.md` に追記する**こと（例: Stripe本番切替、法務ページの実記入、専門家レビュー、Jamf配信、税/インボイス設定など）。
- このファイルは**バージョン管理せず1ファイルを更新し続ける**生きたチェックリスト。完了したら `- [x]` にし、末尾の最終更新日を更新する。
- 「〜はあなた側でやってください」と口頭で伝えるだけで終わらせない。必ず同ファイルに残す。

## エラー体系のドキュメント同期【必須】

- エラー体系（`app/errors.py` の ERROR_CATALOG / classify / AppError、`app/main.py` の共通例外ハンドラ、`app/services/poster.py` の `_friendly`、各ルーターのコード付きエラー）を**変更・追加したら、必ず `docs/エラー体系仕様書_verX.Y.md` も改版する**こと。
- 仕様書は**上書き禁止**。バージョンを上げた新ファイル（例: `エラー体系仕様書_ver1.1.md`）を作り、旧版は履歴として残す。ファイル末尾の「改版履歴」に1行追記する。
- 最新版は常に最大のバージョン番号のファイル。実装と仕様書のコードは必ず一致させる。
