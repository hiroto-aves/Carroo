...
- **実装済みの機能**:
  ...
  - `trabox_form_mapper.py` 全面書き換え: 行ラベル（`.tbx-form-item` + `.label-wrapper`）基準の堅牢なセレクター設計（動的な rc_select ID に依存しない）
  - `trabox.py` フォーム入力処理書き換え: Ant Design 専用操作
    - 発/着 日時: カレンダー（`td[title="YYYY-MM-DD"]`）+ 時/分メニュー、月送り対応
    - 発地/着地: 日本地図型都道府県選択 + 検索型市区町村選択【両方必須】
    - 荷姿=その他 + 荷種（オートコンプリート型、Escape禁止・Tab確定）+ 総重量kg
    - 希望車両: 重量クラス（kg→トン切上げ変換）+ 車種（small_truck→平 等のマッピング）
    - 運賃・高速代（必須ラジオ、既定=支払わない）
  - E2E検証: 実ページで全必須項目入力成功（バリデーションエラーゼロ）を確認
  - 回帰テスト: `test_trabox_fill_form.py`（登録ボタンは押さない安全設計）

- **現在着手中のタスク**:
  - なし（フォーム入力ロジック完成、送信テスト待ち）

- **次にやるべきこと**:
  1. 登録ボタン押下込みの本投稿テスト（テスト荷物の登録→即削除、要ユーザー判断）
  2. 発地/着地に市区町村が必要になったため、Web UI 側の入力フォーム連携確認（pick_location に「東京都港区」形式で市区町村まで入れる）
  3. 卸し日時・高速代・荷種を case_data に追加するスキーマ拡張検討
  4. Cloud Run 環境での動作確認・再デプロイ
