...
## 🆕 2026-08-19 案件変更時のupdate/register自動振り分け（rev carroo-00119-2wq）
- 案件管理画面「投稿する」→「この内容で変更する」（/cases/{id}/update）が、掲載状態に
  関わらず常にupdateタスクを投げていたバグを修正（案件44で発覚: Trabox登録失敗＝未掲載
  なのに、変更ボタンを押すとupdateが飛び「まだ掲載されていないため変更できません
  （E-POST-NOTLISTED）」で永久に失敗し続けていた）。
- 保存前に `get_platform_state` でプラットフォームごとの掲載状態を確認し、
  live→update / それ以外(未投稿・失敗・削除済み)→register に自動振り分けするよう修正。
- ⚠️ user所感「微妙」— 動作は直ったが、UI上「投稿する」ボタンを押すと『変更』画面に
  遷移する導線自体は変わっておらず、体験としてはまだ分かりにくい可能性。次回、
  未掲載時は画面文言を「投稿する」に統一する等のUI整理を検討。
...
- 一括削除バー(#bulk-delete-bar)に `hidden` と `flex` を同じ要素に併記していたため、
  ...
  常時表示される不具合。JSの style.display 直接操作に変更して解消（rev carroo-00115-dl4）。
- 動的HTMLレスポンスに `Cache-Control: no-store` を明示付与。デプロイ後も古い画面が
  ブラウザキャッシュに残り「直したのに反映されない」と誤解される事故を防止
  （rev carroo-00116-9vl）。
- 実運用確認: 管理者が自社の他ユーザーの案件をチェックしても削除できない
  （所有者チェックによる意図的な安全側の仕様）ため、チェックが残り続けて見えていた
  だけと判明。ユーザー絞り込み検索をすれば正常。追加対応は不要と判断。

## 🆕 2026-08-05 Trabox削除のページネーション対応＋extract_cityの地名バグ修正（rev carroo-00113/114）
- **Trabox削除がページネーション未対応で失敗するバグ修正**（rev carroo-00113-92w）:
  一覧に掲載件数が多いと対象が2ページ目以降に流れ、「一覧に見つかりません」で
  何度削除しても失敗する不具合。見つかるまで「次へ」を押してページを送るように修正
  （update_caseはURL直指定のため元々影響なし）。
- **extract_city の地名切り出しバグ修正**（rev carroo-00114-z86）: 「四日市市」「廿日市市」
  「野々市市」のように地名の中に「市」を2回含む場所で、最初の「市」で止まって
  「四日市」等の誤った短縮形になってしまうバグ。WebKitは完全一致必須のため
  `入力エラー: destarea` で登録失敗（Traboxは候補一覧からのフォールバックで
  救われていたため気づきにくかった）。正規表現の最終代替パターンを貪欲マッチに変更し解消。

## 🆕 2026-08-05 荷物一覧からのクイック削除・チェックボックス一括削除（rev carroo-00111〜112）
...
- store.py に `get_platform_states_bulk` を新設（Firestore inフィルタで一括取得、N+1回避）。
  既に両プラットフォームとも削除済み/未投稿の行は、一覧でチェックボックスを無効化し
  「削除済み」表示に変更（rev carroo-00112-wp9）。
...
- 荷物登録ボタンにスピナーを追加したが、実際には既存のグローバル仕組み(window.__busy/__idle、
  ...
  上書きされて見えなくなっていた。独自実装を撤去し共通機構に一本化（data-busy-text属性で
  「登録中…（数十秒かかることがあります）」を指定）。
- data-actボタン(削除/変更/成約にする/グループ一括取下げ等、アプリ全体のクリック委譲)は
  従来busy表示の対象外だった。共通ディスパッチャ自体を拡張し、data-act経由の全ボタンにも
  自動でwindow.__busy/__idleが効くように変更（Promiseを返す非同期処理は完了時に自動idle、
  同期関数はconfirm()キャンセル等の早期returnでもすぐidleに戻る設計）。
- main.py 1箇所に実装されているため、個別ページの修正で消える心配がない。

## 🆕 2026-08-04/05 Trabox/WebKit項目正規化＋荷物登録フォームの重大バグ修正（rev carroo-00108-5gj）

### 正規化仕様書の新規策定
- `docs/Trabox_WebKit項目正規化仕様書_ver1.0/1.1.md` を新規作成。車種形状・重量クラス・荷姿・
  時間区分等の実装済みマッピング整理、荷物区分の21択ドロップダウン化、輸送取扱区分・必要装備の
  新規追加、温度帯の横断的な扱い方針、引越し案件のWebKit自動連動を設計として確定。
  Trabox実フォームのスクリーンショットで現物確認（必要装備=フリーテキスト、荷扱い=非対応）。

### 実装した機能
- 荷物区分（品目）: 自由入力 → WebKit準拠21択ドロップダウンに変更
- 輸送取扱区分（保冷/冷凍/常温等）・必要装備（ヘルメット/安全靴等+自由記入）を新規追加
- 引越し案件チェック時、WebKit側は荷物区分を自動的に「引越貨物」へ強制切替

### 重大バグ修正の連鎖（実運用で連続発覚）
1. **Traboxの重量が「問わず」で登録されるバグ**（本当の原因を今回初めて特定・修正。先週の
   「修正しました」報告は実装が反映されておらず未解決だった）
2. **Trabox重量クラス8t以上が選択できないバグ**: Ant Designの仮想スクロールでDOM未描画のため
   タイムアウト → 見つかるまでスクロールして探す処理に修正
3. **積み/卸し時間のズレ**（例: カレンダー18:40 vs フリーテキスト18:35）: カレンダーの10分刻み
   丸めとフリーテキストの丸めを統一
4. **WebKitのcarkindtype未定義エラー(NameError)**: import整理時のミスでWebKit投稿が全滅
   していたのを修正
5. **輸送取扱区分/必要装備を1個だけ選択するとE-VALIDATIONになるバグ**: `List[str] = Form(None)`
   がFastAPI0.103系で単一選択時にリスト化されない既知の制約。`request.form().getlist()`に変更
6. **登録ボタンを押しても何も起きないバグ→重複登録**: `document.write()`によるクライアント側
   差し替えがCSP nonce不一致で不安定動作。Post-Redirect-Get（実ページ遷移）方式に変更
7. **E-VALIDATIONの該当項目が分からない問題**: `FIELD_LABELS`対応表を新設し、日本語で
   該当項目名を表示するように改善

### ドキュメント更新
- `docs/エラー体系仕様書_ver1.2.md` 新規作成（FIELD_LABELS・PRGパターンを追記）

## 🆕 2026-08-03 Traboxの重量が「問わず」で登録されるバグ修正（rev carroo-00099-fh8）
- 荷物重量(kg)を入れても Trabox の希望車両トン数が「問わず」で登録される不具合を修正。
- 原因: `trabox.py` の weight_class 決定ロジックで、希望車両がデフォルト「問わず」のとき
  `"問わず" in TRUCK_WEIGHT_OPTIONS` が True になり「明示指定」と誤判定→荷物重量からの
  トン数算出(weight_to_class)がスキップされていた（WebKitは常にkgから算出のため正常だった）。
- 修正: `_tw != "問わず"` を条件に追加。「問わず」/未指定なら荷物重量(kg)から算出。
  明示指定(例4t)はそれを尊重。（例: 2000kg→2t, 300kg→軽, 3500kg→4t）

## 🆕 2026-08-03 「複数日程で一括登録」UIの崩れ修正＋ブランド/スマホ対応（rev carroo-00098-c2l）
- チェックボックス周りの巨大な空白の根本原因を修正: ui_shell.py のグローバル
  `input,select,textarea{width:100%;padding;border}` が checkbox/radio にも効いていた。
  checkbox/radio を除外し `width:auto;padding:0` を明示（アプリ全体のチェックボックスに好影響）。
- 複数日程UI(_MULTIDATE_UI)を作り直し: 紫(purple)→ブランドトークン(signal系)、
  日程行をgridレイアウト化（PC横並び／スマホ2列+削除ボタン折返し）、ラベル・本目番号表示、
  最大5日程で追加ボタン自動非表示・削除時に番号振り直し。

## 🆕 2026-07-31 未掲載の変更・取下げエラーを専用コード化＋無駄なリトライ停止（rev carroo-00097-ktw）
- 失敗投稿欄の E-POST-UNKNOWN（案件ID14の変更・取下げ）の原因を調査 → 実体は「掲載中の番号(baggage_no)が
  見つかりません」＝初回投稿が未掲載だったため変更・取下げ対象が無い状態だった。汎用コードに丸められ原因が不明瞭だった。
- **改善①**: `E-POST-NOTLISTED`（まだ掲載されていないため変更・取下げできません）を新設。classify に掲載番号なし
  判定を最優先で追加（errors.py）。→ 原因が一目で分かるメッセージに。
- **改善②**: `NON_RETRYABLE_CODES` を新設し、tasks.py で「全プラットフォームがリトライ不能エラー」なら
  リトライ余地に関わらず即DLQ確定。掲載番号なしは何度リトライしても直らないため、無駄な3回リトライと
  結果メール連投を停止。
- エラー体系仕様書 `docs/エラー体系仕様書_ver1.1.md` を新規作成（ver1.0は履歴として保持）。

## 🆕 2026-07-31 荷物登録フォームの不要なTrabox認証入力欄を削除（rev carroo-00096-k5d）
- 「荷物を出す」画面でトラボックスにチェックを入れると、ID・パスワード入力欄(#trabox-auth)が表示される
  古いUIが残っていた（初期設定機能を作る前の残骸）。この欄は登録処理側で受け取られておらず完全に無視される
  ダミーだった（実際の投稿は初期設定=store.get_credentialsの認証情報を使用）。
- 「初期設定したのにまた認証情報を求められる」というユーザー混乱の原因だったため、入力欄と表示切替JSを削除。

## 🆕 2026-07-30 Trabox利用規約の確認（自動投稿は問題なし・他社情報表示は非実装方針）
- `https://www.trabox.ne.jp/corporate/rules.html` を確認。自動化ツール・ボット・API利用を禁止する条項は無し
  → 自社データの自動投稿（現行Carrooの動作）は規約上問題ないと判断。
- 唯一の関連条項は「情報の運送・物流以外の目的利用／二次利用の禁止」。他社の荷物・車両情報を検索・取得して
  Carroo上に表示する機能はこれに抵触する可能性が高いため、**実装しない方針**として `docs/オーナー作業TODO.md` に記録。
- 詳細・原文引用は `docs/オーナー作業TODO.md`「外部利用サービスの規約確認」セクション参照。

## 🆕 2026-07-30 Jamf Web Clip 配信＋未ログイン時のリダイレクト修正（rev carroo-00095-m6r）

### 実装済み
- **社内リリース：Jamf Now Web Clip 配信 完了**（鈴木さん・小山さんのiPhoneに配信・小山さん実機確認済み）
  - PWAマニフェストのブランド色を旧・青系から現行ブランド（墨×シグナルグリーン）に修正済み
- **未ログイン時に認証必須ページへ直接アクセスした場合のリダイレクト修正**
  - 従来：`{"detail":"Not authenticated"}` というJSONがそのまま黒画面に表示される不具合
    （Jamf Web ClipでDeep link `/dashboard/` を開いた際に発生・小山さんの実機で発覚）
  - 修正：`app/main.py` に `_StarletteHTTPException` 用のハンドラを追加。401かつブラウザの
    ページ遷移（Accept: text/html + Sec-Fetch-Mode: navigate）なら `/auth/login?next=<元のパス>` へ302リダイレクト。
    fetch/XHR経由の401（画面内の非同期処理）は従来通りJSONのまま返すため既存フロントは無影響。
  - `app/routers/auth.py` のログインページが `next` パラメータを受け取り、ログイン成功後に元のページへ遷移
    （オープンリダイレクト対策：`/` 始まり・`//` 始まりでないパスのみ許可）。

### 次にやること
- 鈴木さんの端末が次にオンラインになった時、Web Clipが正しく反映されるか確認
- 引き続き社内での実運用フィードバックを収集
