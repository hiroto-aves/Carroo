...
async def execute_task(request: Request):
    ...
    if not payload.get("case_data") or payload.get("user_id") is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="user_id と case_data は必須です",
        )

    logger.info(
        f"[Tasks] 実行リクエスト受信: case_id="
        f"{payload['case_data'].get('case_id')}"
    )
    results = await execute_posting_task(payload)
